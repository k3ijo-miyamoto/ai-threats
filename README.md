...
> **Disclaimer**
> 本リポジトリは個人プロジェクトであり、所属組織の公式見解・採用技術・運用ポリシーを示すものではありません。
> サンプル設定 ([config/company_assets.example.yaml](config/company_assets.example.yaml)) に登場するAIサービス名は
> 一般的なエンタープライズAIツールの例示であり、特定企業の実利用状況を表すものではありません。
